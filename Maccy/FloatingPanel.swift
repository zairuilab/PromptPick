...
class FloatingPanel<Content: View>: NSPanel, NSWindowDelegate {
  ...
  override func resignKey() {
    ...
    #if DEBUG
    // XCUIAutomation temporarily makes its runner key while it waits for the
    // app to become idle. Keep prompt-flow windows alive during those explicit
    // tests; product-driven close() calls still behave normally.
    if CommandLine.arguments.contains("keep-panel-open-for-testing") {
      return
    }
    #endif

    // SwiftUI sheets make their parent panel resign key before AppKit finishes
    // attaching the sheet. Closing synchronously here tears down both the panel
    // and the sheet, which makes actions such as "New Prompt" look like the app
    // has quit. Wait one run-loop turn so AppKit can establish the relationship,
    // then close only when focus really moved outside this panel hierarchy.
    DispatchQueue.main.async { [weak self] in
      guard let self, self.isPresented else { return }

      let keyWindowBelongsToPanel = NSApp.keyWindow === self
        || NSApp.keyWindow?.sheetParent === self
      let isPresentingTransientWindow = self.attachedSheet != nil
        || keyWindowBelongsToPanel
        || NSApp.alertWindow != nil

      if !isPresentingTransientWindow {
        self.close()
      }
    }
  }
