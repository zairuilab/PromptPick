...
class History: ItemsContainer { // swiftlint:disable:this type_body_length
  ...
  func load() async throws {
    ...
    let retentionDays = Defaults[.historyRetentionDays]
    if retentionDays > 0,
       let cutoff = Calendar.current.date(byAdding: .day, value: -retentionDays, to: .now) {
      let expired = results.filter { $0.pin == nil && $0.lastCopiedAt < cutoff }
      expired.forEach(Storage.shared.context.delete)
      if !expired.isEmpty {
        try? Storage.shared.context.save()
        results.removeAll { item in expired.contains(where: { $0 === item }) }
      }
    }
